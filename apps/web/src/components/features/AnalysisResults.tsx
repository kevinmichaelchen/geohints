...
const ConfidenceBadge = component$<ConfidenceBadgeProps>(({ confidence }) => {
  const colorClass =
    confidence >= 80 ? "bg-green-600" : confidence >= 50 ? "bg-yellow-600" : "bg-gray-600";

  return <span class={`px-2 py-1 rounded text-sm font-medium ${colorClass}`}>{confidence}%</span>;
});
    ...
    <div class="flex items-center justify-between p-4 bg-qwik-dirty-black rounded-lg">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <div class="text-sm text-gray-400">Matched: {result.matchedCharacters.join(", ")}</div>
        </div>
      </div>
      <div class="flex items-center gap-3">
...
export const FoundCharacters = component$<FoundCharactersProps>(({ characters }) => {
...
export const AnalysisResults = component$<AnalysisResultsProps>(({ results }) => {
  if (results.length === 0) {
    return (
