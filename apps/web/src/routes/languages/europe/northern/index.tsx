...
import { SearchFilter, type ScriptFilter } from "~/components/features/SearchFilter";
...
export default component$(() => {
  ...
  const filteredCharacters = useComputed$(() => {
    return filterCharacters(allCharacters, searchQuery.value, scriptFilter.value);
  });
  ...
  return (
    <div class="min-h-screen">
      <PageHeader
        ...
        breadcrumbs={[{ label: "Languages", href: "/languages" }, { label: "Northern Europe" }]}
      />
      ...
      <section class="container mx-auto px-4 pb-16">
        ...
        />
        <ResultsCount showing={filteredCharacters.value.length} total={allCharacters.length} />
        <CharacterGrid characters={filteredCharacters.value} highlightQuery={searchQuery.value} />
      </section>
    </div>
  );
});
