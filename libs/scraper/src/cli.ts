 ...
 */
const PlatformLayer = Layer.mergeAll(NodeHttpClient.layer, NodeFileSystem.layer, NodePath.layer);
  ...
  ({ category, all, source }) =>
    Effect.gen(function* () {
      ...
      if (all) {
        ...
        const manifest =
          src === "geohints" ? yield* scrapeAllGeohints() : yield* scrapeAllGeomastr();
        ...
        const entries =
          src === "geohints" ? yield* scrapeGeohints(cat) : yield* scrapeGeomastr(cat);
        ...
        yield* Effect.log(
          `Done! Scraped ${entries.length} images, manifest now has ${newManifest.entries.length} total entries`,
        );
      } else {
      ...
      const walkDir = async (dir: string): Promise<string[]> => {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            !entry.name.includes("-800w") &&
            !entry.name.includes("-1200w") &&
