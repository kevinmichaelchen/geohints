  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
          ...
          const response = yield* pipe(
            ...
            Effect.flatMap((maybeResponse) =>
              maybeResponse === undefined
                ? Effect.fail(
                    new TimeoutError({ url, timeoutMs: config.timeoutMs })
                  )
      ...
      const fetchHtml = (url: string): Effect.Effect<string, HttpServiceError> =>
        fetchWithRetry(url, "text/html,application/xhtml+xml", (response) =>
          response.text
        )

      const fetchImage = (
        url: string
      ): Effect.Effect<Uint8Array, HttpServiceError> =>
        fetchWithRetry(url, "image/*", (response) =>
          response.arrayBuffer.pipe(
            Effect.map((buffer) => new Uint8Array(buffer as ArrayBuffer))
          )
        )
      ...
      const fetchMany = <A, E>(
        ...
        Stream.fromIterable(urls).pipe(
          Stream.mapEffect(f, { concurrency: config.concurrency })
        )
