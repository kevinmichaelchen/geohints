...
export interface ImageServiceShape {
  ...
  readonly processImage: (
    data: Uint8Array
  ) => Effect.Effect<ProcessedImage, ImageError>
      ...
      const toWebp = (
        ...
        Effect.tryPromise({
          try: async () => {
            ...
            new ImageError("Failed to convert to WEBP", error),
        })
      ...
      const resizeToWidth = (
        ...
        Effect.tryPromise({
          try: async () => {
            ...
            new ImageError(`Failed to resize to ${width}px`, error),
        })
      ...
      const validateWebp = (
        ...
        Effect.tryPromise({
          try: async () => {
            ...
            new ImageError("Failed to validate WEBP format", error),
        })
        ...
        Effect.gen(function* () {
          ...
          const metadata = yield* Effect.tryPromise({
            try: async () => {
              ...
              new ImageError("Failed to read image metadata", error),
          })
